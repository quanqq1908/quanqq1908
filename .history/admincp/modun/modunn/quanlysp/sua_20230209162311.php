...
<form method="POST" action="modunn/quanlysp/xuly.php?idsanpham=<?php echo $_GET['idsanpham']?>" enctype="multipart/form-data">
  <?php
...
<tr>
  ...
  <tr>
   <td>Mã số</td>
    <td><input type="text" name="masp" required value="<?php echo $row['masp']?>"></td>
  </tr>
  <tr>
   <td>Giá sản phẩm</td>
    <td><input type="text" name="giasp" required value="<?php echo $row['giasp']?>"></td>
  </tr>
  <tr>
   <td>Tóm tắt</td>
  ...
  <tr>
   <td>Danh mục sản phẩm</td>
    <td>
      <select class="button"  name="danhmuc">
        <?php
  ...
  <tr>
   <td>Tình trạng</td>
    <td>
      <select class="button" name="tinhtrang">
        <?php
        if ($row['tinhtrang'] == 1) {
          ?>
  ...
  <tr>
    <td colspan="2"><input class="button" type="submit" name="suasanpham" value="Sửa sản phẩm"></td>
  </tr>
</form>
  <?php
  } 
